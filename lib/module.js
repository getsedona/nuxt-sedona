...
export default function module(moduleOptions = {}) {
  ...
  const nuxtMainPath = require.resolve('nuxt')
  const nodeModules = nuxtMainPath.replace('/nuxt/dist/nuxt.js', '')
  ...
  function getComponentFiles(component) {
    ...
    for (const component of components) {
      result.push([component.replace(/\.js|vue$/, ''), component])
    }
    return result
  }
  ...
  for (const component of options.components) {
    ...
    if (componentFiles.length > 0) {
      vueComponents[component] = componentFiles
    }
  }
